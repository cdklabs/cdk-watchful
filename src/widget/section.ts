...
export class SectionWidget extends TextWidget {
  private static toMarkdown(props: SectionWidgetProps) {
    ...
    parts.push('#'.repeat(props.titleLevel ?? 1) + ' ' + props.titleMarkdown);

    // description

    if (props.descriptionMarkdown) {
      parts.push(props.descriptionMarkdown);
    }

    // quick links

    if (props.quicklinks && props.quicklinks.length > 0) {
      parts.push(props.quicklinks.map(link => '[button:' + link.title + '](' + link.url + ')').join(' '));
    }
