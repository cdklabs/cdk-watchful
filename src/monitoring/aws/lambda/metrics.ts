...
export class LambdaMetricFactory {
  metricInvocations(functionName: string) {
    return this.metric(Metrics.Invocations, functionName).with({
  ...
  metricDuration(functionName: string) {
    ...
    return {
      min: baseMetric.with({
        ...
        label: Statistic.MINIMUM,
      }),
      avg: baseMetric.with({
        statistic: Statistic.AVERAGE,
        label: Statistic.AVERAGE,
      }),
      ...
      max: baseMetric.with({
        ...
        label: Statistic.MAXIMUM,
      }),
    };
  }

  metricErrors(functionName: string) {
    return this.metric(Metrics.Errors, functionName).with({
      statistic: Statistic.SUM,
    });
  }

  metricThrottles(functionName: string) {
    return this.metric(Metrics.Throttles, functionName).with({
      statistic: Statistic.SUM,
    });
  }
