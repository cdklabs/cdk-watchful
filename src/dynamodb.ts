...
export class WatchDynamoTable extends Construct {
  ...
  constructor(scope: Construct, id: string, props: WatchDynamoTableProps) {
    ...
    const billingMode =
      (cfnTable.billingMode as dynamodb.BillingMode) ??
    ...
    switch (billingMode) {
      ...
      case dynamodb.BillingMode.PROVISIONED:
        this.createWidgetsForProvisionedTable(
          props.title,
          table,
  ...
  private createWidgetsForProvisionedTable(
    ...
    this.watchful.addAlarm(
      this.createDynamoCapacityAlarm(
        'read',
        readCapacityMetric,
    ...
    this.watchful.addSection(title, {
      links: [
        { title: 'Amazon DynamoDB Console', url: linkForDynamoTable(table) },
      ],
    });

    this.watchful.addWidgets(
      this.createDynamoCapacityGraph(
        'Read',
        readCapacityMetric,
        throughput.readCapacityUnits,
        readCapacityThresholdPercent,
  ...
  private createDynamoCapacityAlarm(
    ...
    const threshold = calculateUnits(
      provisioned,
      percent,
...
function calculateUnits(
  ...
  return (
    provisioned *
    ((percent === undefined ? DEFAULT_PERCENT : percent) / 100) *
    period.toSeconds()
  );
}
